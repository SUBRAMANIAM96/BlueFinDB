#!/bin/bash
# =====================================================================
# BlueFinDB v3.0 - TEST QC SUITE
# INTERNAL TESTING QC LOGIC
# This script contains the final QC logic for the Test Environment.
# =====================================================================
...
if [ $# -ne 6 ]; then
    ...
    echo "Usage: ./Bluefindb_test_qc.sh <BLAST_DB_NAME> <FASTA_FILE> <QC_LOG_FILE> <FWD_PRIMER> <REV_PRIMER> <VERSION>"
    exit 1
fi
...
MANIFEST_FILE="BlueFinDB_Test_Manifest.txt" # Test-specific manifest

# --- 2. START LOGGING ---
echo -e "\n[STAGE 3] Starting final database QC (TEST MODE VSEARCH) and re-index..." | tee -a "$PATCH_LOG"
echo "--- Starting Final QC and Build (BlueFinDB $SCRIPT_VERSION [TEST]) ---" > "$QC_LOG_FILE"
...
if [ ! -s "$TEMP_N_FILTER" ] && [ "$PRE_QC_COUNT" -gt 0 ]; then
    echo "   [QC WARNING] VSEARCH N-filter produced an empty file. This should not happen. Skipping N-filter." | tee -a "$QC_LOG_FILE" "$PATCH_LOG"
...
echo "[QC STEP 5] Building final BLAST database (TEST)..." | tee -a "$QC_LOG_FILE" "$PATCH_LOG"
...
echo "--- Writing Reproducibility Manifest (TEST) ---" | tee -a "$PATCH_LOG"
echo "### BlueFinDB $SCRIPT_VERSION [TEST RUN] Reproducibility Manifest ###" > "$MANIFEST_FILE"
echo "" >> "$MANIFEST_FILE"
...
# [MODIFIED]: Changed hardcoded '150-550' to dynamic text to match new multi-marker logic
